...
<%
    ...
    rs = stmt.executeQuery("SELECT COUNT(*) FROM list");
    int allPosts = 0;
    if (rs.next()) {
...
<div class="wrap">
    <header>
        <h1><a href="./list_all.jsp">HOME</a></h1>
    ...
    <div class="box">
        <nav>
            <ul>
                ...
                <li><a href="./list_my.jsp?page=1" class="select">내 게시글</a></li>
        ...
        <div class="content">
            ...
            <table class="table table-striped">
                ...
                <%
                    ...
                    stmt = con.createStatement();
                    rs = stmt.executeQuery("SELECt * FROM list WHERE author='" + user + "'");
                %>
                <% if (!rs.next()){ %>
                    <tr>
                ...
                <% } else {%>
                    <%
                        rs.close();
                        rs = stmt.executeQuery("SELECt * FROM list WHERE author='" + user + "' ORDER BY idx DESC");
                        int i = 1;
                    %>
                    <% while (rs.next()) { %>
                        <%
                        ...
                        <tr>
                            ...
                            <td><a href="setList.jsp?idx=<%=rs.getString("idx")%>"><%=rs.getString("title")%></a></td>
                ...
                <% if (rs.next()) { %>
                    <%
                        ...
                        stmt = con.createStatement();
                        rs = stmt.executeQuery("SELECT COUNT(*) FROM list WHERE author='" + author + "'");
                    %>
                    <% if (rs.next()) { %>
                        <% if (rs.getInt(1) > 5) { %>
                            <ul class="pageNavigation">
                                <% if (pageNum > 1) { %>
                                    <a href="list_my.jsp?page=<%=pageNum-1%>"><&nbsp;</a>
                                <% } else { %>
                                    <&nbsp;&nbsp;
                                <% } %>
                                <% for (int i=1; i<=maxPage; i++) {%>
                                &nbsp;  <li><a href="list_my.jsp?page=<%=i%>"><%=i%></a></li>&nbsp;
                                <% } %>
                                <% if (pageNum < maxPage) { %>
                                    <a href="list_my.jsp?page=<%=pageNum+1%>">&nbsp;></a>
                                <% } else { %>
